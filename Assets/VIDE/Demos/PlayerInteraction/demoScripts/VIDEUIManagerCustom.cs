...
public class VIDEUIManagerCustom : MonoBehaviour
{
    ...
    bool PreConditions(VIDE_Assign dialogue)
    {
        ...
        if (VD.isActive) //Stuff we check while the dialogue is active
        {
            ...
            if (!data.isPlayer)
            {
                if (dialogue.assignedDialogue == "CatDialog")
                {
                    if (data.extraVars.ContainsKey("CheckGlass"))
                    {
                        if (Inventory.Instance.Items[0]?.Name == "Glass")
                        {
                            VD.SetNode(5);
                        }
                        else
            ...
            {
                /*if (dialogue.assignedDialogue == "Cat")
                {
                    if (Inventory.Instance.Items[0]?.Name == "Glass")
                    {
                        VD.SetNode(5);
